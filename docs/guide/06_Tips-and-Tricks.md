There are a lot of tips we can offer here. If you've already read everything else in the guide, the next place to look is the [extras](https://github.com/uhs-robert/hyprvim/tree/main/extras).

The extras also include Submaps for "Resize", "Move", and even [keyboard-driven mouse movement](https://github.com/uhs-robert/hyprvim/tree/main/extras/wl-kbptr). There's a lot that can be learned from another configuration setup, even if you already have many of these things.

## Using HyprVim Keybinds Outside of Vim Modes

In [Hyprland Basics](https://github.com/uhs-robert/hyprvim/tree/main/extras/hyprland-basics), you can learn how to use HyprVim actions outside of Vim mode. This is useful when you want the same behavior from your regular Hyprland binds.

### Recommended Keybinds to Grab

- `Set Mark`
- `Jump Mark`
- `Open Vim Anywhere` (`gn` / `gi`)
- `Open Command Mode` (`:`)

These are all easy to wire into `keymaps` or into your own Hyprland bindings after `require("lua/plugins/hyprvim").setup(...)`.
